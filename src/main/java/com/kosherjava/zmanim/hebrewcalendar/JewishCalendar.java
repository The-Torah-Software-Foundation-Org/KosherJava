...
public class JewishCalendar extends JewishDate {
    ...
    public static enum Parsha {
        ...
        VAYAKHEL_PEKUDEI,
        /**
         * The double <em>parsha</em> of Tazria
         * &amp; Metzora
         */
        TAZRIA_METZORA,
        /**
     ...
     */
    public JewishCalendar(ZonedDateTime date) {
        super(date.toLocalDate());
    }
     ...
     *
     * @param jewishYear       the Jewish year
     * @param jewishMonth      the Jewish month. The method expects a 1 for <em>Nissan</em> ... 12 for <em>Adar</em> and 13 for
     *                         <em>Adar II</em>. Use the constants {@link #NISSAN} ... {@link #ADAR} (or {@link #ADAR_II} for a
     *                         leap year Adar II) to avoid any confusion.
     * @param jewishDayOfMonth the Jewish day of month. If 30 is passed in for a month with only 29 days (for example {@link #IYAR},
     *                         or {@link #KISLEV} in a year that {@link #isKislevShort()}), the 29th (last valid date of the month)
     *                         will be set.
     * @param inIsrael         whether in Israel. This affects <em>Yom Tov</em> calculations
     */
     ...
     */
    public ZonedDateTime getMoladAsDate() {
        ...
        ZonedDateTime cal = ZonedDateTime.of(
                LocalDateTime.of(
                        ...
                        (int) moladSeconds,
                        (int) TimeUnit.MILLISECONDS.toNanos((long) (1000L * (moladSeconds - (long) moladSeconds)))
                ),
                geo.getTimeZone().toZoneId()
        );

        // subtract local time difference of 20.94 minutes (20 minutes and 56.496 seconds) to get to Standard time
        return cal.plus(-1 * (int) geo.getLocalMeanTimeOffset(), ChronoUnit.MILLIS);
    }
    ...
    public ZonedDateTime getTchilasZmanKidushLevana3Days() {
        return getMoladAsDate().plusHours(72); // 3 days after the molad
    }
    ...
    public ZonedDateTime getTchilasZmanKidushLevana7Days() {
        return getMoladAsDate().plusHours(168); // 7 days after the molad
    }
    ...
    public ZonedDateTime getSofZmanKidushLevanaBetweenMoldos() {
        ...
        return getMoladAsDate()
                .plusHours((24 * 14) + 18)
                .plusMinutes(22)
                .plusSeconds(1)
                .plusNanos(TimeUnit.MILLISECONDS.toNanos(666));
    }
    ...
    public ZonedDateTime getSofZmanKidushLevana15Days() {
        return getMoladAsDate().plusHours(24 * 15); //15 days after the molad. Add it as hours, not days, to avoid DST/ST crossover issues.
    }
