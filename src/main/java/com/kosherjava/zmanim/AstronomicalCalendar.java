 ...
 * <code>{@link java.time.ZonedDateTime}</code> is expected and {@link Long#MIN_VALUE} when a <code>long</code> is expected. The
 ...
 * ZonedDateTime sunrise = ac.getSunrise();
 * </pre>
 *
...
public class AstronomicalCalendar implements Cloneable {
    ...
    /**
     * The getSunrise method Returns a <code>ZonedDateTime</code> representing the
     ...
     *
     * @return the <code>ZonedDateTime</code> representing the exact sunrise time. If the calculation can't be computed such as
    ...
    public ZonedDateTime getSunrise() {
        ...
        if (Double.isNaN(sunrise)) {
            ...
            return getZonedDateTimeFromTime(sunrise, true);
        }
    }
     ...
     *
     * @return the <code>ZonedDateTime</code> representing the exact sea-level sunrise time. If the calculation can't be computed
     ...
     */
    public ZonedDateTime getSeaLevelSunrise() {
     ...
     *
     * @return The <code>ZonedDateTime</code> of the beginning of civil twilight using a zenith of 96&deg;. If the calculation
     * can't be computed, <code>null</code> will be returned. See detailed explanation on top of the page.
     * @see #CIVIL_ZENITH
     */
    public ZonedDateTime getBeginCivilTwilight() {
     ...
     *
     * @return The <code>ZonedDateTime</code> of the beginning of nautical twilight using a zenith of 102&deg;. If the calculation
     * can't be computed <code>null</code> will be returned. See detailed explanation on top of the page.
     * @see #NAUTICAL_ZENITH
     */
    public ZonedDateTime getBeginNauticalTwilight() {
     ...
     *
     * @return The <code>ZonedDateTime</code> of the beginning of astronomical twilight using a zenith of 108&deg;. If the calculation
     * can't be computed, <code>null</code> will be returned. See detailed explanation on top of the page.
     * @see #ASTRONOMICAL_ZENITH
     */
    public ZonedDateTime getBeginAstronomicalTwilight() {
        return getSunriseOffsetByDegrees(ASTRONOMICAL_ZENITH);
    }

    /**
     * The getSunset method Returns a <code>ZonedDateTime</code> representing the
     ...
     *
     * @return the <code>ZonedDateTime</code> representing the exact sunset time. If the calculation can't be computed such as in
    ...
    public ZonedDateTime getSunset() {
        ...
        if (Double.isNaN(sunset)) {
            ...
            return getZonedDateTimeFromTime(sunset, false);
        }
    }
     ...
     *
     * @return the <code>ZonedDateTime</code> representing the exact sea-level sunset time. If the calculation can't be computed
     * such as in the Arctic Circle where there is at least one day a year where the sun does not rise, and one
     * where it does not set, a <code>null</code> will be returned. See detailed explanation on top of the page.
     * @see AstronomicalCalendar#getSunset
     * @see AstronomicalCalendar#getUTCSeaLevelSunset 2see {@link #getSunset()}
     */
    public ZonedDateTime getSeaLevelSunset() {
     ...
     *
     * @return The <code>ZonedDateTime</code> of the end of civil twilight using a zenith of {@link #CIVIL_ZENITH 96&deg;}. If the
     * calculation can't be computed, <code>null</code> will be returned. See detailed explanation on top of the page.
     * @see #CIVIL_ZENITH
     */
    public ZonedDateTime getEndCivilTwilight() {
     ...
     *
     * @return The <code>ZonedDateTime</code> of the end of nautical twilight using a zenith of {@link #NAUTICAL_ZENITH 102&deg;}. If
     * the calculation can't be computed, <code>null</code> will be returned. See detailed explanation on top of the
     * page.
     * @see #NAUTICAL_ZENITH
     */
    public ZonedDateTime getEndNauticalTwilight() {
     ...
     *
     * @return the <code>ZonedDateTime</code> of the end of astronomical twilight using a zenith of {@link #ASTRONOMICAL_ZENITH
     * 108&deg;}. If the calculation can't be computed, <code>null</code> will be returned. See detailed
     * explanation on top of the page.
     * @see #ASTRONOMICAL_ZENITH
     */
    public ZonedDateTime getEndAstronomicalTwilight() {
     ...
     * offset as a <code>long</code> and calls {@link #getTimeOffset(ZonedDateTime, long)}.
     *
     * @param time   the start time
     * @param offset the offset in milliseconds to add to the time
     * @return the {@link java.time.ZonedDateTime}with the offset added to it
     */
    public static ZonedDateTime getTimeOffset(ZonedDateTime time, double offset) {
     ...
     * @param time   the start time
     * @param offset the offset in milliseconds to add to the time.
     * @return the {@link java.time.ZonedDateTime} with the offset in milliseconds added to it
     */
    public static ZonedDateTime getTimeOffset(ZonedDateTime time, long offset) {
        if (time == null || offset == Long.MIN_VALUE) {
            return null;
        }
        return time.plus(offset, ChronoUnit.MILLIS);
    }
     ...
     *                     parameter.
     * @return The {@link java.time.ZonedDateTime} of the offset after (or before) {@link #getSunrise()}. If the calculation
     * can't be computed such as in the Arctic Circle where there is at least one day a year where the sun does
     * not rise, and one where it does not set, a <code>null</code> will be returned. See detailed explanation
     * on top of the page.
     */
    public ZonedDateTime getSunriseOffsetByDegrees(double offsetZenith) {
        double dawn = getUTCSunrise(offsetZenith);
        if (Double.isNaN(dawn)) {
            return null;
        } else {
            return getZonedDateTimeFromTime(dawn, true);
        }
    }
     ...
     * @return The {@link java.time.ZonedDateTime}of the offset after (or before) {@link #getSunset()}. If the calculation can't
     * be computed such as in the Arctic Circle where there is at least one day a year where the sun does not
     * rise, and one where it does not set, a <code>null</code> will be returned. See detailed explanation on
     * top of the page.
     */
    public ZonedDateTime getSunsetOffsetByDegrees(double offsetZenith) {
    ...
    public AstronomicalCalendar(GeoLocation geoLocation) {
        setCalendar(ZonedDateTime.now(geoLocation.getTimeZone().toZoneId()));
     ...
     * @see #getSunrise()
     * @see #getSunset()
     * @see #getTemporalHour(ZonedDateTime, ZonedDateTime)
     * @see #getTemporalHour(ZonedDateTime, ZonedDateTime)
     */
     ...
     * @see #getTemporalHour()
     */
    public long getTemporalHour(ZonedDateTime startOfday, ZonedDateTime endOfDay) {
        if (startOfday == null || endOfDay == null) {
            return Long.MIN_VALUE;
        }
        return (endOfDay.toEpochSecond() - startOfday.toEpochSecond()) / 12;
    }
     ...
     *
     * @return the <code>ZonedDateTime</code> representing Sun's transit. If the calculation can't be computed such as when using
     ...
     *
     * @return the <code>ZonedDateTime</code> of astronomical solar midnight. If the calculation can't be computed such as
    ...
    public ZonedDateTime getSolarMidnight() {
        ...
        clonedCal.setCalendar(clonedCal.getCalendar().plusDays(1));
        return getTimeOffset(getSunTransit(), (clonedCal.getSunTransit().toEpochSecond() - getSunTransit().toEpochSecond()) / 2);
    }
     ...
     * @return the <code>ZonedDateTime</code> representing Sun's transit. If the calculation can't be computed such as in the
     * Arctic Circle where there is at least one day a year where the sun does not rise, and one where it does
     * not set, <code>null</code> will be returned. See detailed explanation on top of the page.
     */
    public ZonedDateTime getSunTransit(ZonedDateTime startOfDay, ZonedDateTime endOfDay) {
        long temporalHour = getTemporalHour(startOfDay, endOfDay);
        return getTimeOffset(startOfDay, temporalHour * 6);
    }

    /**
     * A method that returns a <code>ZonedDateTime</code> from the time passed in as a parameter.
     *
     * @param time      The time to be set as the time for the <code>ZonedDateTime</code>. The time expected is in the format: 18.75
     *                  for 6:45:00 PM.
     * @param isSunrise true if the time is sunrise and false if it is sunset
     * @return The ZonedDateTime.
     */
    protected ZonedDateTime getZonedDateTimeFromTime(double time, boolean isSunrise) {
        ...
        ZonedDateTime adjustedCalendar = getAdjustedCalendar();
        LocalDate localDate = LocalDate.of(adjustedCalendar.getYear(),
                adjustedCalendar.getMonth(),
                adjustedCalendar.getDayOfMonth());
        ...
        return ZonedDateTime.of(localDate, LocalTime.of(hours, minutes, seconds, (int) calculatedTime), getGeoLocation().getTimeZone().toZoneId());
    }
    ...
    public double getSunriseSolarDipFromOffset(double minutes) {
        ZonedDateTime offsetByDegrees = getSeaLevelSunrise();
        ZonedDateTime offsetByTime = getTimeOffset(getSeaLevelSunrise(), -(minutes * MINUTE_MILLIS));

        BigDecimal degrees = new BigDecimal(0);
        BigDecimal incrementor = new BigDecimal("0.0001");

        while (offsetByDegrees == null || ((minutes < 0.0 && offsetByDegrees.compareTo(offsetByTime) < 0) ||
                (minutes > 0.0 && offsetByDegrees.compareTo(offsetByTime) > 0))) {
            if (minutes > 0.0) {
    ...
    public double getSunsetSolarDipFromOffset(double minutes) {
        ZonedDateTime offsetByDegrees = getSeaLevelSunset();
        ZonedDateTime offsetByTime = getTimeOffset(getSeaLevelSunset(), minutes * MINUTE_MILLIS);
        BigDecimal degrees = new BigDecimal(0);
     ...
     * @return the ZonedDateTime representing the local mean time (LMT) for the number of hours passed in. In Lakewood, NJ, passing 12
     * (noon) will return 11:56:50am.
     * @see GeoLocation#getLocalMeanTimeOffset()
     */
    public ZonedDateTime getLocalMeanTime(double hours) {
        if (hours < 0 || hours >= 24) {
            throw new IllegalArgumentException("Hours must between 0 and 23.9999...");
        }
        return getTimeOffset(getZonedDateTimeFromTime(hours - getGeoLocation().getTimeZone().getRawOffset()
    ...
    private ZonedDateTime getAdjustedCalendar() {
		...
		return getCalendar().plusDays(offset);
    }
    ...
    public void setGeoLocation(GeoLocation geoLocation) {
        ...
        this.calendar = this.calendar.withZoneSameInstant(geoLocation.getTimeZone().toZoneId());
    }
    ...
    public void setCalendar(ZonedDateTime calendar) {
        ...
        if (getGeoLocation() != null) {// if available set the Calendar's timezone to the GeoLocation TimeZone
            this.calendar = getCalendar().withZoneSameInstant(getGeoLocation().getTimeZone().toZoneId()); //TODO should this be withZoneSameLocal?
        }
    }
    ...
    public Object clone() {
        ...
        clone.setCalendar(ZonedDateTime.ofInstant(getCalendar().toInstant(), getCalendar().getZone()));
